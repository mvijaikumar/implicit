...
class AlternatingLeastSquares(RecommenderBase):
    ...
    def fit(self, item_users):
        ...
        for iteration in range(self.iterations):
            ...
            solver(Ciu, self.item_factors, self.user_factors, self.regularization,
                   num_threads=self.num_threads)
            log.debug("finished iteration %i in %.3fs", iteration, time.time() - s)
    ...
    def _fit_gpu(self, Ciu, Cui):
        ...
        Ciu = implicit.cuda.CuCSRMatrix(Ciu)
        Cui = implicit.cuda.CuCSRMatrix(Cui)
        ...
        for iteration in range(self.iterations):
            s = time.time()
            solver.least_squares(Ciu, Y, X, self.regularization)
            solver.least_squares(Cui, X, Y, self.regularization)
            log.debug("finished iteration %i in %.3fs", iteration, time.time() - s)
